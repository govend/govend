...
func TestFixImports(t *testing.T) {
	...
	findImport = func(pkgName string, symbols map[string]bool, priority []string) (string, bool, error) {
...
`)
	...
	got, rename, err := findImportGoPath("bytes", map[string]bool{"Buffer2": true}, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	got, rename, err = findImportGoPath("bytes", map[string]bool{"Missing": true}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if got != "" || rename {
