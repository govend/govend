...
func fixImports(fset *token.FileSet, f *ast.File, priority []string) (added []string, err error) {
	...
	for pkgName, symbols := range refs {
		...
		go func(pkgName string, symbols map[string]bool) {
			ipath, rename, err := findImport(pkgName, symbols, priority)
...
func findImportGoPath(pkgName string, symbols map[string]bool, priority []string) (string, bool, error) {
	...
	for importPath := range pkgs {
		for _, prefix := range priority {
			if len(importPath) >= len(prefix) {
				if importPath[:len(prefix)] == prefix {
